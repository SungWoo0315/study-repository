...
> 파일 업로드가 없는곳에서 파일 날려도 에러는 안나지만, 파일 이름값만 넘어간다.  
> 파일업로드에는 항상 form 태그나 ajax 상단에,  
> `document.name=폼태그name값.enctype="multipart/form-data";` 붙여 준다.  
...
### # <문7> 자바스크립트에서 `document.폼태그name값.submit();` 코딩의 의미는?? 
